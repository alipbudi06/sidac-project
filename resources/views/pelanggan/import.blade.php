...
@section('content')
    ...
    <form action="{{ route('pelanggan.import.process') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div>
            <label for="file_pelanggan">Pilih File (Excel/CSV)</label>
            <input type="file" id="file_pelanggan" name="file_pelanggan" required>
        </div>
